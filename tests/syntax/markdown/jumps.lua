...
local document = {
  '# Jump history', '', '| Key | Value |', '|---|---|',
  '| item | ' .. string.rep('wrapped content ', 40) .. '|', '', 'Last paragraph',
}
...
assert(#vim.fn.getjumplist()[1] == 1, 'Opening the rendered view added a source jump')
...
local function jump(keys, target, position)
  ...
  vim.wait(30)
  assert(vim.api.nvim_get_current_buf() == target, 'Jump reached the wrong buffer: ' .. keys)
  if position then
    assert(vim.deep_equal(vim.api.nvim_win_get_cursor(0), position),
      'Jump lost its cursor: ' .. keys .. ' ' .. vim.inspect(vim.api.nvim_win_get_cursor(0)))
  end
end

for _ = 1, 3 do
  jump('<C-o>', before, { 2, 3 })
  assert(vim.api.nvim_buf_is_valid(rendered), 'Leaving Markdown deleted the forward destination')
  jump('<C-i>', rendered, rendered_position)
  assert(vim.wo.foldmethod == 'manual' and vim.wo.wrap, 'Jumping into the preview lost its window options')
end
...
assert(vim.wait(200, function() return vim.api.nvim_get_current_buf() == rendered end, 5),
  'Reopening the source replaced the preview and invalidated jump destinations')
assert(vim.deep_equal(vim.api.nvim_win_get_cursor(0), rendered_position),
  'Reopening the source lost its projected position')
preview.toggle()
assert(vim.api.nvim_get_current_buf() == source and not vim.api.nvim_buf_is_valid(rendered),
  'Explicit source mode retained the hidden preview')
local source_history = vim.fn.getjumplist()
preview.toggle()
preview.toggle()
assert(vim.deep_equal(vim.fn.getjumplist(), source_history), 'Source toggles changed native jump history')
...
assert(not vim.api.nvim_buf_is_valid(hidden_preview), 'Deleting the source leaked its hidden preview')
