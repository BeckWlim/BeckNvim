...
local original_object = assert(features.cached_projection(cached_objects, object_key, 1, build_object))
local moved_object = assert(features.cached_projection(cached_objects, object_key, 3, build_object))
assert(builds == 1 and moved_object.start_row == 3 and moved_object.end_row == 4
  and moved_object.rows[2].source_row == 3 and original_object.rows[2].source_row == 1,
  'Moving an unchanged object rebuilt it or changed its cached source mapping')
assert(moved_object.rows[2].chunks == original_object.rows[2].chunks
  and moved_object.rows[2].spans == original_object.rows[2].spans,
  'Moving an unchanged object copied its rendered content')
assert(#features.changed_ranges(original_object.rows, moved_object.rows) == 0,
...
assert(#changed_ranges == 2 and changed_ranges[1].old_count == 1 and changed_ranges[2].old_count == 1,
  'Separate prose edits included unchanged objects in the buffer patch')
local element_plan = features.plan_elements({
  { key = 'completed', start_row = 10 }, { key = 'running' }, { key = 'new' },
  { key = 'new' }, { key = 'over limit' },
}, { completed = true, removed = true }, { running = true }, 3)
assert(element_plan.by_key.completed.start_row == 10 and not element_plan.by_key.removed
  and not element_plan.by_key['over limit'] and #element_plan.render == 1 and element_plan.render[1].key == 'new',
