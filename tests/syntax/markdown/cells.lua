...
local hidden_link_offset = assert(linked_cell:find('hidden', 1, true)) - 1
local trailing_text_offset = assert(linked_cell:find('tail', 1, true)) - 1
assert(
  markdown.cell_fragment(linked_cell, hidden_link_offset, 7) == 2
    and markdown.cell_fragment(linked_cell, trailing_text_offset, 7) == 3,
