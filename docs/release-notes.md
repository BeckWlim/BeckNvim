...
The release includes `lazy-lock.json` for plugin revisions. External tools and Mason-managed
language servers are installed separately; Termaid defaults to its `main` branch unless
`BECKNVIM_TERMAID_REF` selects a revision. A BeckNvim tag alone does not pin the whole toolchain.
