...
local function preferred_column_widths(rows, column_count)
  ...
  for column = 1, column_count do
    ...
    for _, row in ipairs(rows) do
      widths[column] = math.max(widths[column], display_width(cell_display_text(row.cells[column])))
    end
  end
  return widths
end
...
local function render_table(parsed_table, width)
  ...
  local limit = M.table_width_limit(math.max(width - parsed_table.start_column, 1))
  local content_width = math.max(limit - M.cell_margins.left - M.cell_margins.right, 1)
  local widths, gap = M.allocate_widths(content_width, parsed_table.column_count,
    preferred_column_widths(parsed_table.rows, parsed_table.column_count))
  local total_width = M.cell_margins.left + M.cell_margins.right + (#widths - 1) * gap
  for _, column_width in ipairs(widths) do total_width = total_width + column_width end
  local rows = {}
  ...
  for row_index, row in ipairs(parsed_table.rows) do
    ...
    for fragment = 1, height do
      local chunks = { { indentation .. string.rep(' ', M.cell_margins.left), table_highlights[row.role] } }
      local spans = {}
      local output_byte = #chunks[1][1]
      for column, cell_lines in ipairs(cells) do
        local characters = cell_lines[fragment] or {}
        local alignment = row_index == 1 and 'center' or parsed_table.alignments[column]
        local padded, left_padding = padded_chunks(characters_chunks(characters, table_highlights[row.role]),
          widths[column], alignment, table_highlights[row.role])
        local character_byte = output_byte + left_padding
        for _, character in ipairs(characters) do
          spans[#spans + 1] = {
            first = character_byte,
            last = character_byte + #character.text,
            source_column = row.cell_ranges[column].start_column + (character.source_start or 0),
          }
        ...
        if column < #widths then
          chunks[#chunks + 1] = { string.rep(' ', gap), table_highlights[row.role] }
          output_byte = output_byte + gap
        end
      end
      chunks[#chunks + 1] = { string.rep(' ', M.cell_margins.right), table_highlights[row.role] }
      rows[#rows + 1] = { chunks = chunks, source_row = row.source_row, spans = spans }
    end
    if row_index == 1 then
      decoration(string.rep('─', total_width), table_highlights.rule, parsed_table.delimiter_row)
    elseif row_index < #parsed_table.rows then
      decoration(string.rep('┈', total_width), table_highlights.row_rule, row.source_row)
    end
  end
...
function M.project(context)
  ...
  local layout = { context.width, M.table_width_limit(context.width), M.cell_margins.left, M.cell_margins.right }
  local cached = context.cache and context.cache.tables
  if cached and cached.changedtick == changedtick and vim.deep_equal(cached.layout, layout) then
